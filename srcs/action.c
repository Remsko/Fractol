...
/*   Updated: 2018/05/13 16:29:33 by rpinoit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
inline int	mouse_hook(int key, int x, int y, t_env *env)
{
	if (key == LEFT_CLICK || key == SCROLL_UP || key == RIGHT_CLICK ||
			key == SCROLL_DOWN || key == PAD_PLUS || key == PAD_MINUS)
	{
		env->cur_f->x1 += ((((double)x - env->win_h / 2.0) / env->win_w / 2.0)
				/ env->cur_f->zoom_r * 10.0) / 15.0;
		env->cur_f->x2 += ((((double)x - env->win_h / 2.0) / env->win_w / 2.0)
				/ env->cur_f->zoom_r * 10.0) / 15.0;
		env->cur_f->y1 += ((((double)y - env->win_w / 2.0) / env->win_h / 2.0)
				/ env->cur_f->zoom_r * 10.0) / 15.0;
		env->cur_f->y2 += ((((double)y - env->win_w / 2.0) / env->win_h / 2.0)
				/ env->cur_f->zoom_r * 10.0) / 15.0;
...
int			motion_hook(int x, int y, t_env *env)
{
	if (x >= 0 && y >= 0 && x <= (int)env->win_w && y <= (int)env->win_h
			&& env->active_m == 'Y')
	{
		env->cur_f->c.r = (double)x / (double)env->win_w * 2 - 1;
		env->cur_f->c.i = (double)y / (double)env->win_h * 2 - 1;
		mlx_fractal(env);
	}
	return (0);
}
